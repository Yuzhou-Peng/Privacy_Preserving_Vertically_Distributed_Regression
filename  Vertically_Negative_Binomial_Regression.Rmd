...
opt_result <- NB_dual_gradient_ascent(u_init = rep(-1, 10000), X = as.matrix(X), y = simulation_data$Y, r = 17, m = 10000, lambda = 0.02)
...
X_design <- nmda %>%
  select(Seizures, CO_24h_M0, AQI_M0, NO2_24h_M0, SO2_24h_M0)
...
mRS_opt_u_NB <- NB_dual_gradient_ascent(u_init = rep(-1, length(y_mRS)), X = as.matrix(X_design), y = y_mRS, r = 1.7, m = length(y_mRS), lambda = 0.02)$u


mRS_dual_beta_NB <- t(X_design) %*% mRS_opt_u_NB / (nrow(X_design)  * 0.02)
```
