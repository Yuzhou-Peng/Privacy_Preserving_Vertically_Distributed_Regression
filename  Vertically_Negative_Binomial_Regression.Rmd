...
```{r}
gradient_ascent <- function(u_init, X, y, r, m, lambda, 
  ...
  list(u = u, iter = iter, value = D_obj(u, X, y, r, m, lambda))
}


opt_result <- gradient_ascent(u_init = rep(-1, 10000), X = as.matrix(X), y = simulation_data$Y, r = 17, m = 10000, lambda = 0.02)

u_opt <- opt_result$u

t(X) %*% u_opt / (10000  * 0.02)
```
...
coef_ridge <- coef(cv_fit, s = "lambda.min")
coef_ridge


lambda_hat <- predict(cv_fit, newx = X_chemicals, s = "lambda.min", type = "response")

dispersion_ratio <- sum((y_n_cases - lambda_hat)^2 / lambda_hat) / (length(y_n_cases) - ncol(X_chemicals))
dispersion_ratio
...
```{r}
glm.nb(n_cases ~., data = df_model)
```

```{r}

dual_nb_fit_res <- gradient_ascent(u_init = rep(-1, nrow(X_chemicals)), X = as.matrix(X_chemicals), y = y_n_cases, r = 1.77, m = nrow(X_chemicals), lambda = 0.02)

t(X_chemicals) %*% dual_nb_fit_res$u / (nrow(X_chemicals)  * 0.02)
```
