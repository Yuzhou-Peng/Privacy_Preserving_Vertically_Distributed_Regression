...
```{r}
opt_u_backtrack <- NB_dual_gradient_ascent(u_init = rep(-1, 10000),X = as.matrix(X), y = simulation_data$Y, r = 5, m = 1e4, lambda = 0.02, max_iter = 1e6)$u

t(X) %*% opt_u_backtrack / (1e4  * 0.02)
```

```{r}

opt_u <- NB_dual_gradient_ascent(u_init = rep(-1, 400), gram_mat = (as.matrix(X) %*% t(as.matrix(X)) ), y = simulation_data$Y, r = 5, m = 400, lambda = 0.02, max_iter = 5e5)$u

as.matrix(t(X)) %*% opt_u / (400  * 0.02)
```
