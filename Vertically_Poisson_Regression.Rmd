...
poisson_dual_gradient_ascent <- function(Y, X, lambda, u0 = NULL,
  ...
  for (iter in 1:max_iter){
    ...
    history <- rbind(history, data.frame(iter=iter, obj=f, grad_norm=gnorm, step=step))

    if (verbose && (iter %% 50 == 0)) {
      message(sprintf("iter=%d  obj=%.10f  grad_norm=%.3e  step=%.3e", iter, f, gnorm, step))
    }
  }

  list(u = u, objective = f, history = history)
}
...
X_design <- nmda %>%
  select(Age, Seizures, CO_24h_M0, NO2_24h_M0, PM2.5_24h_M0, SO2_24h_M0)

X_design <- as.matrix(X_design)
X_design <- apply(X_design, 2, as.numeric)


mRS_opt_u <- poisson_dual_gradient_ascent(Y = y_mRS, X = X_design, lambda = 0.2)$u

mRS_dual_beta <- t(X_design) %*% mRS_opt_u / (nrow(X_design)  * 0.2)
```
